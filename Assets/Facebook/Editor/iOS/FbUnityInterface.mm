...
frictionlessRequests:(bool)_frictionlessRequests {
  ...
  if (self.session.state == FBSessionStateCreatedTokenLoaded) {
    ...
    UnitySendMessage("Facebook", "OnInitComplete", "");
  }
  return self;
}
      ...
      [FBRequestConnection startForMeWithCompletionHandler:
       ^(FBRequestConnection *connection, id result, NSError *error) {
        ...
        if(self.isInitializing) {
          UnitySendMessage("Facebook", "OnInitComplete", strRes.c_str());
          self.isInitializing = NO;
        } else {
          UnitySendMessage("Facebook", "OnLogin", strRes.c_str());
        }
      }];
    }
      break;
...
-(void)logout {
  ...
  UnitySendMessage("Facebook", "OnLogout", "");
}
...
void RequestCompletionHelper(int requestId, bool isError, const char *payload) {
  ...
  UnitySendMessage("Facebook", "OnRequestComplete", temp.c_str());
}
